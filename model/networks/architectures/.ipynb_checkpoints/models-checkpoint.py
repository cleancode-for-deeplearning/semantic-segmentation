...
from model.networks.architectures.base_modules import *
from model.networks.architectures.constructors.resnet import *
from model.networks.architectures.encoders import *
from model.networks.architectures.decoders import *

class UNet(nn.Module):
	def __init__(self, opt):
		super(UNet, self).__init__()
		enc_type = opt.encType
		dec_type = opt.decType
		if(enc_type == 'unet'):
			self.encoder, nf = UNetEncoder(opt, 64), get_nf('unet')
		elif(enc_type == 'res1'): # Doesn't work, needs a new design
			self.encoder, nf = ResNetEncoder(opt, 'stride1'), get_nf(opt.encMode)
		elif(enc_type == 'res2'): # Doesn't work, needs a new design
			self.encoder, nf = ResNetEncoder(opt, 'stride2'), get_nf(opt.encMode)
...
class PSPNet(nn.Module):
	def __init__(self, opt):
		...
		if(enc_type == 'res1'):
			self.encoder, nf = ResNetEncoder(opt, 'astrous1'), get_nf(opt.encMode)
