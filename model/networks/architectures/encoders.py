...
from model.networks.architectures.constructors.resnet import constructor

class UNetEncoder(nn.Module):
	def __init__(self, opt, nf):
		super(UNetEncoder, self).__init__()
		ic, oc, norm_type, act_type = \
			opt.ic, opt.oc, opt.normType, opt.actType
...
class ResNetEncoder(nn.Module):
	def __init__(self, opt, mode):
		...
		ic, oc, norm_type, act_type, constructor_mode = \
			opt.ic, opt.oc, opt.normType, opt.actType, opt.encMode
		self.mode = mode
