...
from search_hyperparams.options import options
from model.dataloaders.seg_dataloader import Dataset
from evaluate import Segmentation_Trainer
...
for epoch in range(epoch_num):
	for i, data in enumerate(tqdm(trn_dl)):
		...
		if(i % opt.visFreq == 0):
			...
			img_pth = os.path.join(opt.visPth, img_fn)
			save_cnt += 1
			cv2.imwrite(img_pth, plot_img)

trainer.save(opt.modelPth)